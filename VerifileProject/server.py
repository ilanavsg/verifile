...
class Server:
    def __init__(self):
        ...
        # We need a lock because multiple threads (clients) will access the DB and shared lists simultaneously
    ...
    def _update_client_list(self):
        ...
        # Use lock when reading for GUI update
        with self.db_lock:
    ...
    def handle_sell_action(self, client_socket, user_id):
        ...
        try:
            ...
            with self.db_lock:
                ...
                price = float(self.encryptor.receive_encrypted_message(client_socket))
                ...
                self.db_manager.update_row(
                    "clients", "user_id", user_id, ["role"], ["seller"]
                )
        ...
        try:
            ...
            with self.db_lock:
                ...
                self.db_manager.insert_row(
                    ...
                    "(file_id, user_id, action, hash_value, signature, verified, created_at)",
                    "(%s,%s,%s,%s,%s,%s,%s)",
                    (file_id, user_id, "verify", hash_hex, signature_hex, verified, datetime.now())
                )

            self.encryptor.send_encrypted_message(client_socket, "VALID" if verified else "INVALID")
            ...
            with self.db_lock:
                ...
                if existing:
                    ...
                    if bcrypt.checkpw(password.encode(), db_password.encode()):
                        ...
                        self.update_gui_log(f"Client {user_id} - Auth attempt failed")
        ...
        while True:
            ...
            with self.conn_lock:
                ...
                if rows:
                    ...
                    if ddos_status == 1:
                        self.update_gui_log(f"Blocked IP tried to connect: {ip}")
                        client_socket.close()
                        continue

                if self.total_connections >= MAX_TOTAL_CONNECTIONS:
                    self.update_gui_log(f"Max total connections reached")
                    client_socket.close()
                    continue
                ...
                if len(self.connections[ip]) >= MAX_CONNECTIONS_PER_IP:
                    self.update_gui_log(f"DDOS detected from IP {ip}")
                    for sock in self.connections[ip]:
                        try: sock.close()
                        except: pass
                    self.connections[ip].clear()

                    if rows:
                        latest_session = max(rows, key=lambda x: x[0])
                        user_id = latest_session[0]
                        with self.db_lock:
                            self.db_manager.update_row("clients", "user_id", user_id, ["ddos_status"], [True])
                    client_socket.close()
                    continue

                self.connections[ip].append(client_socket)
                self.total_connections += 1
    ...
    def create_gui(self):
        ...
        logo = Image.open(r"C:\Users\Cyber_User\Downloads\VerifileLogo.png").resize((400, 400))
